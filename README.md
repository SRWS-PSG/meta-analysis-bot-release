...
2. ボットが存在するSlackチャンネルでCSVファイルを共有
3. ボットはCSVを分析し、適切な列が見つかった場合にメタ解析を実行
4. ユーザーは自然な日本語で分析の意図を伝える
5. Geminiがユーザーの意図を理解し、適切な分析手法を自動選択
6. 結果はスレッド内に共有され、スレッド内で会話コンテキストが維持される

## 強化された対話フロー

### 従来の方法
```
ユーザー: "サブグループ解析をお願いします"
Bot: "サブグループ列を選択してください..."
```

### 改善後の自然な対話
```
ユーザー: "オッズ比でランダム効果モデル、サブグループは年齢グループで"
Gemini: 意図を理解 → 効果量・モデル・サブグループを自動抽出
Bot: "以下の設定でメタアナリシスを開始します：効果量OR、ランダム効果、サブグループ年齢グループ"
```

### 多層フォールバック戦略

1. **第1層**: Gemini分析 → 成功時は自動実行
2. **第2層**: Gemini失敗 → 従来の手動ダイアログ
3. **第3層**: 互換性エラー → 基本テンプレートで実行
...
- [ユーザー対話フロー](docs/user_interaction_flow.md)
- [スレッドコンテキスト管理](docs/thread_context_management.md)

## 開発ロードマップ

詳細は[ROADMAP.md](ROADMAP.md)ファイルを参照してください。
