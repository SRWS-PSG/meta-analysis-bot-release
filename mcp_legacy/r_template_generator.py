...
"""
    ...
    def _generate_rma_code(self, analysis_params: Dict[str, Any]) -> str:
        method = analysis_params.get("method", "REML")
    ...
    def _generate_subgroup_code(self, analysis_params: Dict[str, Any]) -> str:
        ...
        for subgroup_col in subgroup_columns:
            subgroup_test_model = self._safe_format(
                self.templates["rma_with_mods"],
                method=method, mods_formula=f"factor({subgroup_col})",
...
"""
    ...
    def _generate_plot_code(self, analysis_params: Dict[str, Any], output_paths: Dict[str, str]) -> str:
        plot_parts = []
        data_cols = analysis_params.get("data_columns", {})
        ai_col = data_cols.get("ai", "")
        bi_col = data_cols.get("bi", "") 
            ...
            for sg_col in subgroup_columns:
                ...
                plot_parts.append(
                    self._safe_format(
                        ...
                        measure_for_plot=analysis_params.get("measure", "RR"),
                        ai_col=ai_col, bi_col=bi_col, ci_col=ci_col, di_col=di_col,
                        res_for_plot_model_name="res_for_plot",
                        row_h_in_placeholder=self.PLOT_ROW_H_IN,
                        base_h_in_placeholder=self.PLOT_BASE_H_IN,
                        plot_width_in_placeholder=self.PLOT_WIDTH_IN,
                        plot_dpi_placeholder=self.PLOT_DPI,
    ...
    def _generate_save_code(self, analysis_params: Dict[str, Any], output_paths: Dict[str, str]) -> str:
    ...
    if (exists("res_subgroup_test_{subgroup_col}") && !is.null(res_subgroup_test_{subgroup_col})) {{
        summary_list$subgroup_moderation_test_{subgroup_col} <- list(
            ...
            QMp = res_subgroup_test_{subgroup_col}$QMp, df = res_subgroup_test_{subgroup_col}$p,
            summary_text = paste(capture.output(summary(res_subgroup_test_{subgroup_col})), collapse = "\n")
        )
    }}
    if (exists("res_by_subgroup_{subgroup_col}") && !is.null(res_by_subgroup_{subgroup_col})) {{
        subgroup_results_list_{subgroup_col} <- list()
        for (subgroup_name in names(res_by_subgroup_{subgroup_col})) {{
            current_res_sg <- res_by_subgroup_{subgroup_col}[[subgroup_name]]
            subgroup_results_list_{subgroup_col}[[subgroup_name]] <- list(
                ...
                summary_text = paste(capture.output(summary(current_res_sg)), collapse = "\n")
            )
        }}
...
""")
        regression_json_str = ""
        if analysis_params.get("moderator_columns"):
            regression_json_str = f"""
    if (exists("res") && !is.null(res$beta) && length(res$beta) > 1 && "{' + '.join(analysis_params.get('moderator_columns', []))}" != "") {{
        moderator_results <- list()
        # res$beta can be NULL if the model is intercept-only, even if moderators were specified but removed (e.g. all NA)
        if (!is.null(res$beta) && nrow(res$beta) > 1) {{ # Check if res$beta has more than one row (intercept + moderators)
            for (i in 2:nrow(res$beta)) {{ # Start from 2 to skip intercept
        ...
        summary_list$meta_regression_results <- list(
            moderators = moderator_results, R2 = res$R2,
            QE_residual = res$QE.f, QEp_residual = res$QEp.f,
            summary_text = paste(capture.output(summary(res)), collapse = "\n")
        )
    }}
"""
        egger_json_str = ""
        if output_paths.get("funnel_plot_path"):
            # additional_objects_to_save.append("egger_test_res") # Assuming regtest is run and saved to egger_test_res
            egger_json_str = """
    if (exists("egger_test_res") && !is.null(egger_test_res)) {
        summary_list$egger_test <- list(
            ...
            summary_text = paste(capture.output(egger_test_res), collapse = "\n")
        )
    }
"""
        additional_objects_str = ""
        if additional_objects_to_save:
            # Ensure 'res' is not duplicated if already included by default
            unique_additional_objects = list(set(additional_objects_to_save) - set(["res"]))
            if unique_additional_objects:
                 additional_objects_str = ", " + ", ".join(unique_additional_objects)
        ...
        if analysis_params.get("moderator_columns"):
            bubble_plot_prefix = output_paths.get("bubble_plot_path_prefix", "bubble_plot")
            for mod_col in analysis_params.get("moderator_columns", []):
                safe_mod_col_name = "".join(c if c.isalnum() else "_" for c in mod_col)
                bubble_plot_path_specific = f"{bubble_plot_prefix}_{safe_mod_col_name}.png".replace('\\\\','/')
                generated_plots_r_list.append(f'list(label = "bubble_plot_{safe_mod_col_name}", path = "{bubble_plot_path_specific}")')
        generated_plots_r_code = f"summary_list$generated_plots <- list({', '.join(generated_plots_r_list)})"
    ...
    def generate_full_r_script(self, 
        ...
        logger.info(f"データサマリー: {data_summary}")
        ...
        plot_model_method = analysis_params.get("method", "REML")
        rma_for_plot_code = self._safe_format(
            self.templates["rma_basic"], method=plot_model_method,
        ).replace("res <-", "res_for_plot <-")
        script_parts.append(rma_for_plot_code)

        rma_code = self._generate_rma_code(analysis_params)
        script_parts.append(rma_code)

        subgroup_cols = analysis_params.get("subgroup_columns", [])
        if subgroup_cols:
            subgroup_code = self._generate_subgroup_code(analysis_params)
            script_parts.append(subgroup_code)

        # Egger's test (if funnel plot is requested)
        if output_paths.get("funnel_plot_path"):
            script_parts.append("egger_test_res <- tryCatch(regtest(res), error = function(e) NULL)")


        plot_code = self._generate_plot_code(analysis_params, output_paths)
        ...
        if sensitivity_variable and sensitivity_value:
        ...
        save_code = self._generate_save_code(analysis_params, output_paths)
        script_parts.append(save_code)
        
        full_script = "\n\n".join(filter(None, script_parts))
        logger.info(f"生成されたRスクリプト:\n{full_script}")
        return full_script
...
if __name__ == '__main__':
    ...
    test_data_summary = {
        "columns": ["author", "year", "tpos", "tneg", "cpos", "cneg", "ablat", "alloc", "gender"],
        "shape": [13, 9]
    }
    test_output_paths = {
    ...
    test_csv_path = "path/to/your/data.csv"
    ...
    test_analysis_params_yi = {
        "measure": "yi", "method": "DL",
        "data_columns": {
            "yi": "effect_value", "vi": "variance_value", "study_label": "study_name"
        }
    }
    test_data_summary_yi = {
        "columns": ["study_name", "effect_value", "variance_value"], "shape": [10, 3]
    }
    r_script_yi = generator.generate_full_r_script(
        test_analysis_params_yi, test_data_summary_yi, test_output_paths, test_csv_path
    )
    print("\n--- Generated R Script (Pre-calculated yi/vi) ---")
    print(r_script_yi)
