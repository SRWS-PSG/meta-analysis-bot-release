...
Jane,30,F"""
        ...
        assert '効果量' in result['reason'] or '研究' in result['reason']
...
Study1,5.2,1.1,30,4.8,1.2,28"""
        ...
        assert 'OR' in binary_result['suggested_analysis']['effect_type_suggestion'] or \
               'RR' in binary_result['suggested_analysis']['effect_type_suggestion']
        assert 'SMD' in continuous_result['suggested_analysis']['effect_type_suggestion'] or \
               'MD' in continuous_result['suggested_analysis']['effect_type_suggestion']
...
Study1,0.5,0.1"""
        ...
        # Then: 不十分と判定される
        assert result['is_suitable'] == False
        assert '研究数' in result['reason'] or '最低' in result['reason']
...
研究2,15,60,8,55"""
        ...
        # 日本語列名でも検出できる
        assert '介入群イベント' in str(result) or '対照群イベント' in str(result)
