...
dat <- read.csv('/tmp/tmpvpndajir.csv', na.strings = c('NA', 'na', 'N/A', 'n/a', ''))
...
png('final_test_forest_plot.png', width=plot_width_in_val, height=plot_height_in_main, units="in", res=plot_dpi_val, pointsize=9)
tryCatch({{
...
# ファンネルプロット作成
png('final_test_funnel_plot.png', width=2400, height=2400, res=300, pointsize=9)
tryCatch({{
    funnel(res)
...
summary_list$generated_plots_paths <- list(list(label = "forest_plot_overall", path = "final_test_forest_plot.png"), list(label = "forest_plot_subgroup_Region", path = "forest_plot_subgroup_Region.png"), list(label = "funnel_plot", path = "final_test_funnel_plot.png"))
...
json_output_file_path <- "final_test_results.json"
tryCatch({
...
tryCatch({
    save(res, res_for_plot, res_by_subgroup_Region, res_subgroup_test_Region, file='final_test_results.RData') 
    print(paste("RData saved to:", 'final_test_results.RData'))
